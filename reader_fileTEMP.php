...
$files = scandir(__DIR__ . './input_data/');
$csv = '';
for ($i = 2; $i < count($files); $i++) {
    $csv .= file_get_contents(__DIR__ . './input_data/' . $files[$i]);
}
...
echo "WRONG.<br>";
p($wrongEmails);
// echo "notEmptyString.<br>";
// p($notEmptyString);

// Запись корректных Email
$buffer = fopen(__DIR__ . './output_data/Correct_emails.csv', 'w');
fputs($buffer, chr(0xEF) . chr(0xBB) . chr(0xBF));
fputcsv($buffer, $correctEmail, "\n");
fclose($buffer);

// Запись ошибочных Email
$buffer = fopen(__DIR__ . './output_data/Wrong_emails.csv', 'w');
fputs($buffer, chr(0xEF) . chr(0xBB) . chr(0xBF));
fputcsv($buffer, $wrongEmails, "\n");
fclose($buffer);

$strstr1 = 'sv@xn----7sbf7ab7adi.xn--p1ai';
echo $strstr1 . '<br>';
echo $encoded;
// $punycode = 'sv@xn----7sbf7ab7adi.xn--p1ai';
